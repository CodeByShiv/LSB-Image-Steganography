...
Status do_encoding(EncodeInfo * encInfo)
{
	...
	if(copy_remaining_img_data(encInfo -> fptr_src_image, encInfo -> fptr_stego_image) == e_success)
		...
		printf("Copying remaining data to destination file -----------------> failed :(\n");
...
Status read_and_validate_encode_args(char *argv[], EncodeInfo *encInfo)
{
	...
	if(data[0] == 0X42 && data[1] == 0X4d)
		...
		printf("BMP file validation ----------------> failed :(\n");
...
{
    char buffer[54];
    fread(buffer, 1, 54, p1); // read 54 bytes of header data and copy it in buffer
    fwrite(buffer, 1, 54,p2); // write the 54 bytes of header data from buffer to p2(ouput.bmp) 
    return e_success;
}
